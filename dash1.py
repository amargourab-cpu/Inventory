...
class Gauge(ctk.CTkCanvas):
    def __init__(self, master, count, title, size=140, **kwargs):
        ...
        if count <= 300:
            ...
            # Auto-calculate a clean scale above the count (leaving ~20% headroom)
            raw_step = count / 8.0 
...
class Dashboard(ctk.CTkToplevel):
    ...
    def check_queue(self):
        if self.fetched_data is not None:
            # Data is ready, stop loading animation
    ...
    def show_enlarged_popup(self, title, data, type_flag="card", color_map=None):
        ...
        ctk.CTkButton(pop, text="CLOSE VIEW", fg_color="#ff6f6f", height=40, font=("Arial", 12, "bold"), command=pop.destroy).pack(pady=30)
    ...
    def show_custodian_drilldown(self, oem):
        ...
        ctk.CTkButton(pop, text="← BACK", fg_color="#e74c3c", text_color="white", font=("Arial", 12, "bold"), command=self.close_drilldown_popup).pack(pady=(0, 10))
    ...
    def show_package_custodian_drilldown(self, oem, package):
        ...
        ctk.CTkButton(pop, text="← BACK", fg_color="#e74c3c", text_color="white", font=("Arial", 12, "bold"), command=lambda: self.show_package_drilldown(oem)).pack(pady=(0, 10))
        scroll = ctk.CTkScrollableFrame(pop, fg_color="transparent")
        scroll.pack(fill="both", expand=True, padx=20, pady=20)
