...
def resolve_log_paths():
    db_root = globals().get("DB_ROOT")
    candidates = []

    if db_root:
        candidates.extend([
            os.path.join(db_root, "Logs"),
            os.path.join(db_root, "logs"),
            os.path.join(db_root, "Activity Logs"),
            os.path.join(db_root, "ActivityLogs"),
        ])

    legacy_path = "/Users/Gourab.Palui/Library/CloudStorage/OneDrive-SharedLibraries-Aptiv/ACS-India - Common Folder/Inventory Database/Logs"
    candidates.extend([
        legacy_path,
        os.path.expanduser("~/Library/CloudStorage/OneDrive-SharedLibraries-Aptiv/ACS-India - Common Folder/Inventory Database/Logs"),
        os.path.expanduser("~/OneDrive/ACS-India - Common Folder/Inventory Database/Logs"),
        os.path.join(os.path.expanduser("~"), ".aptiv_inventory_logs"),
    ])
    ...
    for entry in logs:
        try:
            ...
            if isinstance(ts, str):
                log_date = datetime.strptime(ts, "%Y-%m-%d %H:%M:%S")
...
class MultiSelectDropdown(ctk.CTkFrame):
    """Custom multi-select dropdown with checkboxes for filtering.
    Supports adding a custom entry inside the dropdown and optional
    persistence via a `persist_key` (saved to filter_options.json).
    """
    ...
    def _add_custom(self):
        ...
        if not val:
            return
        if val in self.all_options:
            self.selected_options.add(val)
            if val in self.item_vars:
                self.item_vars[val].set("on")
            self.update_btn_text()
            return
        self.all_options.append(val)
        self.selected_options.add(val)
        if self.menu_window:
            scroll_children = [w for w in self.menu_window.winfo_children() if isinstance(w, ctk.CTkScrollableFrame)]
            if scroll_children:
                scroll = scroll_children[0]
                var = ctk.StringVar(value="on")
                self.item_vars[val] = var
                cb = ctk.CTkCheckBox(scroll, text=str(val), variable=var, onvalue="on", offvalue="off", command=lambda o=val: self.toggle_item(o))
                cb.pack(anchor="w", padx=15, pady=3)
        try:
            if getattr(self, 'persist_key', None):
                save_filter_option(self.persist_key, val)
        except Exception:
            pass
        self._new_var.set("")
        self.update_btn_text()
...
class CustomerOwnedVDR(ctk.CTkToplevel):
    ...
    def update_item(self):
        if self.selected_index is not None:
            self.data_list[self.selected_index] = {"vdr": self.vdr_e.get().strip(), "package": self.pkg_e.get().strip(), "status": self.st_e.get().strip(), "customer": self.cust_e.get().strip()}
            self.save_all_data()
            self.update_filter_options()
            self.clear_fields()
            log_activity("Customer Owned VDR", f"Updated VDR sl no: {self.vdr_e.get().strip()}")

    def delete_item(self):
        if self.selected_index is not None:
            deleted_vdr = str(self.data_list[self.selected_index].get("vdr", ""))
...
class AptivInventory(ctk.CTkToplevel):
    ...
    def change_text_size(self, choice):
        size_map = {
            "Small Text": (12, 13, 30),
            "Medium Text": (14, 15, 38),
            "Large Text": (18, 19, 48)
        }
        if choice not in size_map: return
        base_size, hd_size, row_h = size_map[choice]
        ...
        hd_bg = "#e2e8f0" if "light" in mode else "#2d3748"
        hd_fg = "#1a202c" if "light" in mode else "#dbe7ff"
        ok_col = "#276749" if "light" in mode else "#38A169"
        nok_col = "#9b2c2c" if "light" in mode else "#E53E3E"

        style = ttk.Style(self)
        style.configure("Custom.Treeview", rowheight=row_h, font=("Helvetica", base_size, "italic"), foreground=fg_col, background=bg_col, fieldbackground=bg_col)
        style.configure("Custom.Treeview.Heading", font=("Helvetica", hd_size, "bold"), background=hd_bg, foreground=hd_fg)
        
        self.tree.tag_configure("OK", foreground=ok_col, font=("Helvetica", base_size, "italic"))
        self.tree.tag_configure("NOK", foreground=nok_col, font=("Helvetica", base_size, "italic"))
    ...
    def setup_ui(self):
        ...
        ctk.CTkButton(top_btn_f, text="Close View", width=140, height=35, fg_color="#E53935", hover_color="#D32F2F", command=self.destroy).pack(side="right", padx=(0, 10))
        
        f_container = ctk.CTkFrame(self, corner_radius=12, border_width=1); f_container.pack(fill="x", padx=40, pady=5)
        
        ctk.CTkLabel(f_container, text="Filters", font=("Arial Bold", 15), text_color=("#1f6aa5", "#5da9e9")).pack(pady=(5, 0))
        ...
        self.text_size_var = ctk.StringVar(value="Medium Text")
        self.text_size_cb = ctk.CTkComboBox(top_row, values=["Small Text", "Medium Text", "Large Text"], width=120, height=30, variable=self.text_size_var, command=self.change_text_size, state="readonly")
        self.text_size_cb.pack(side="right", padx=(0, 10))
        ...
        ctk.CTkLabel(self, text="Data Entry/Update/Delete", font=("Arial Bold", 15), text_color=("#1f6aa5", "#5da9e9")).pack(pady=(10, 0))
        ...
        ctk.CTkButton(btn_bar, text="Clear", fg_color="#e74c3c", width=90, command=self.clear_inputs).pack(side="left", padx=2)
        ...
        table_container.pack(fill="both", expand=True, padx=40, pady=(30, 10))

        columns = ["sl"] + self.col_keys
        self.tree = ttk.Treeview(table_container, columns=columns, show="headings", selectmode="browse", style="Custom.Treeview")
        ...
        self.change_text_size(self.text_size_var.get())
        
        self.details_frame = ctk.CTkFrame(self, corner_radius=10)
        self.details_frame.pack(fill="x", padx=40, pady=(30, 0))
        ...
        if hasattr(ctk, "CTkTextbox"):
            self.remarks_box = ctk.CTkTextbox(self.details_frame, width=1480, height=60, state="disabled", corner_radius=8, font=("Arial", 14))
        else:
            self.remarks_box = tk.Text(self.details_frame, width=180, height=3, wrap="word", bd=0, highlightthickness=0, font=("Arial", 14))
    ...
    def delete_entry(self):
        if self.selected_index is not None and 0 <= self.selected_index < len(self.data_list):
            ...
            vdr_id = self.inputs["vdr"].get()
            self.data_list = [i for i in self.data_list if str(i.get("vdr")) != vdr_id]
        self.set_syncing(True)
        with open(MOD3_DATA_FILE, "w") as f: json.dump(self.data_list, f, indent=4)
        self.set_syncing(False)
        self.update_filter_options()
        self.refresh_oem_custodian_options()
        self.clear_inputs()
        deleted_vdr = self.inputs["vdr"].get().strip()
...
class InventoryApp(ctk.CTk):
    def __init__(self):
        ...
        if not os.path.exists(self.image_path):
            fallback = "/Users/gourab.palui/Desktop/Projects/Asset/Launchpage.jpg"
            if os.path.exists(fallback):
                self.image_path = fallback
            else:
                print(f"⚠️ WARNING: Background image not found at either:\n1. {self.image_path}\n2. {fallback}\nPlease check the filename and extension (.jpg vs .png)")
    ...
    def resizer(self, event=None):
        ...
        if win_w == self.current_width and win_h == self.current_height: 
            return
            
        self.current_width, self.current_height = win_w, win_h
        ...
        if os.path.exists(self.image_path):
            try:
                ...
                left = (new_w - win_w) // 2
                top = (new_h - win_h) // 2
                final = resized.crop((left, top, left + win_w, top + win_h))
    ...
    def change_database_path(self):
        ...
        messagebox.showinfo("Reset", "Database path cleared. The app will now close. Please restart to select the new folder.")
        self.destroy()
        sys.exit()
    ...
    def setup_ui_buttons(self):
        ...
        ctk.CTkButton(self, text="Show Analytics Dashboard", **style, command=self.open_analytics).place(relx=0.05, rely=0.30, anchor="nw")
        ctk.CTkButton(self, text="Customer Owned Devices", **style, command=lambda: self.open_popup(CustomerOwnedVDR)).place(relx=0.05, rely=0.41, anchor="nw")
        ctk.CTkButton(self, text="Aptiv Inventory", **style, command=lambda: self.open_popup(AptivInventory)).place(relx=0.05, rely=0.52, anchor="nw")
        
        db_style = {"width": 150, "height": 35, "font": ("Arial Bold", 12), "fg_color": "#e67e22", "hover_color": "#d35400", "corner_radius": 8}
        ctk.CTkButton(self, text="⚙️ Change Database", **db_style, command=self.change_database_path).place(relx=0.98, rely=0.03, anchor="ne")
